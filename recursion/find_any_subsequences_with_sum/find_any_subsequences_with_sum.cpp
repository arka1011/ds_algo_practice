...
bool subSqs(int nums[], int n, int target_sum, int& sum, std::vector<std::vector<int>>& res, std::vector<int>& temp, int index) {
    if (index == n) {
        if (sum == target_sum) {
            res.push_back(temp);
            return true;
    ...
    sum+=nums[index];
    if (subSqs(nums,n,target_sum,sum,res,temp,index+1))
        return true;
    sum-=nums[index];
    temp.pop_back();
    if (subSqs(nums,n,target_sum,sum,res,temp,index+1))
